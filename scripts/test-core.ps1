...
Write-Host "`nTesting Sepolia connection..." -ForegroundColor Cyan
try {
    $response = Invoke-WebRequest -Uri "$env:SEPOLIA_URL/eth/v1/beacon/genesis" -Method Get -ErrorAction Stop
    Write-Host "✅ Sepolia connection test passed" -ForegroundColor Green
} catch {
    Write-Host "❌ Sepolia connection test failed: $_" -ForegroundColor Red
}
...
Write-Host "`nTesting contract address format..." -ForegroundColor Cyan
...
Write-Host "`nTesting Qwen API key format..." -ForegroundColor Cyan
if ($env:QWEN_API_KEY -match '^sk-or-v1-[a-f0-9]+$') {
    Write-Host "✅ Qwen API key format is valid" -ForegroundColor Green
} else {
    Write-Host "❌ Qwen API key format is invalid" -ForegroundColor Red
}
